    ...
    if (Array.isArray(ctors)) {
        for (let ctor of ctors) {
            ...
            for (let i = 0; i < this.ctors.length; i++) {
                if (this.ctors[i].name === ctor.name) this.ctors[i] = ctor;
            }
        }
    } else {
        for (let entry of Object.entries(ctors)) {
            for (let i = 0; i < this.ctors.length; i++) {
                let [name, ctor] = entry;
                if (!ctor) continue;
                if (this.ctors[i].name === name) this.ctors[i] = ctor;
...
KVIN.prototype.userCtors =
    {}; /**< name: implementation for user-defined constructors that are not props of global */
        ...
        switch (typeof po.ctr) {
            case 'string':
                if (!po.ctr.match(/^[A-Za-z_0-9$][A-Za-z_0-9$]*$/)) {
                    if (
                        this.constructorAllowlist &&
                        this.constructorAllowlist.indexOf(po.ctr) === -1
                    ) {
...
KVIN.prototype.unprepare$object = function unprepare$object(seen, po, position) {
    ...
    if (typeof po.ctr === 'string' && !po.ctr.match(/^[1-9][0-9]*$/)) {
        if (this.userCtors.hasOwnProperty(po.ctr)) constructor = this.userCtors[po.ctr];
        else constructor = eval(po.ctr); /* pre-validated! */ // eslint-disable-line
    } else {
        constructor = this.ctors[po.ctr];
    }

    if (po.hasOwnProperty('args')) o = construct(constructor, po.args);
    else if (po.hasOwnProperty('arg')) o = new constructor(po.arg);
    else o = new constructor(); // eslint-disable-line
...
KVIN.prototype.unprepare$Array = function unprepare$Array(seen, po, position) {
    ...
    if (po.hasOwnProperty('isl')) {
        for (let prop in po.isl) {
            ...
            let els = Array.isArray(island.arr)
                ? island.arr
                : this.unprepare$Array(seen, island.arr, [position, 'isl', prop].join('.'));
...
KVIN.prototype.isPrimitiveLike = function isPrimitiveLike(o, seen) {
    if (o === null || typeof o === 'string' || typeof o === 'boolean') return true;

    if (typeof o === 'number') return Number.isFinite(o);

    if (typeof o !== 'object') return false;

    if (o.constructor === this.standardObjects.Object && Object.keys(o).length === 0) return true;

    if (
        o.constructor === this.standardObjects.Array &&
        o.length === 0 &&
    ...
    if (
        o.constructor !== this.standardObjects.Object &&
        o.constructor !== this.standardObjects.Array
    )
        return false;
    ...
    for (let prop in o) {
        ...
        if (seen.indexOf(o[prop]) !== -1) return false;
        if (!this.isPrimitiveLike(o[prop], seen)) return false;
    }
...
function prepare$Error(o) {
    ...
    for (let prop of ['code', 'stack', 'lineNumber', 'fileName'])
        if (o.hasOwnProperty(prop)) ret.ps[prop] = o[prop];
    for (let prop in o) if (o.hasOwnProperty(prop)) ret.ps[prop] = o[prop];
...
KVIN.prototype.prepare = function prepare(seen, o, where) {
    ...
    if (typeof o.toKVIN === 'function') return Object.assign(ret, o.toKVIN(o, this));
    else if (typeof o.toJSON === 'function') ret.arg = o.toJSON();
    else if (o.toString !== this.standardObjects.Object.prototype.toString) ret.arg = o.toString();
    ...
    for (let prop in o) {
        ...
        switch (typeof o[prop]) {
            ...
            case 'object':
                if (o[prop] !== null) {
                    if (
                        typeof o[prop].constructor !== 'undefined' &&
                        o[prop].constructor !== this.standardObjects.Object &&
                        o[prop].constructor.constructor !== this.standardObjects.Object &&
                        o[prop].constructor !== this.standardObjects.Function &&
                        o[prop].constructor.constructor !== this.standardObjects.Function &&
                        o[prop].constructor !== this.standardObjects.Function &&
                        o[prop].constructor.constructor.name !==
                            'Function' /* vm context issue /wg aug 2020 */
                    ) {
                        throw new Error(
                            `Cannot serialize property ${where}.${prop} - multiple inheritance is not supported.`
                        );
                    }
            ...
            default:
                throw new TypeError(
                    'Cannot serialize property ' + prop + ' which is a ' + typeof o[prop]
                );
        }
    }
...
KVIN.prototype.prepare$Array = function prepare$Array(seen, o, where) {
    ...
    if (keys.length !== o.length) {
        ...
        for (let j = 0; j < keys.length; j++) {
            let key = keys[j];
            let idx = +key;
            if (idx < j && pa.arr.hasOwnProperty(idx)) {
                /* test order for speed */
                continue;
            }
            if (typeof idx === 'number' && o.hasOwnProperty(idx + 1)) {
                ...
                if (island.arr.length >= this.scanArrayThreshold) {
                    let tmp = this.prepare(
                        seen,
                        island.arr,
                        where + '.' + 'isl@' + (j - island.arr.length)
                    );
...
KVIN.prototype.prepare$ArrayBuffer16 = function prepare$ArrayBuffer16(o) {
    ...
    if (ret.ctr === -1) ret.ctr = o.constructor.name;
...
KVIN.prototype.prepare$ArrayBuffer = function prepare$ArrayBuffer(o) {
    ...
    if (
        this.tune === 'speed' ||
        this.tune === 'size' ||
        o.byteLength < this.typedArrayPackThreshold
    ) {
    ...
    for (const key in marshalledObject) {
        ...
        switch (typeof marshalledObject[key]) {
            case 'object':
                 ...
                 */
                marshalledObject[key] = await this.marshalAsync(marshalledObject[key], true);
                break;
            default:
                break;
        }
